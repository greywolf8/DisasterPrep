...
public class SMAPApiClient {
    ...
    public void downloadSoilMoistureFile(String localFilePath, String username, String password) throws IOException {
        ...
        long fileAgeHours = -1;
        if (file.exists()) {
            fileAgeHours = (System.currentTimeMillis() - file.lastModified()) / (1000 * 60 * 60);
        }

        // If file exists and is less than 24 hours old, skip download
        if (file.exists() && fileAgeHours < 24) {
            System.out.println("Recent SMAP file found, skipping download.");
            return;
        }
